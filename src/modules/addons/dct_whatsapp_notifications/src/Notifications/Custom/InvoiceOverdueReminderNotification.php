...
/**
 * Code: InvoiceOverdueReminder
 *
 * Instant notification: fires off WHMCS's InvoicePaymentReminder hook, for
 * reminders sent about an invoice that's already PAST its due date (WHMCS
 * uses the same reminder hook/cron for both upcoming and overdue reminders,
 * configured under Setup > Automation Settings > Invoice/Late Fees).
 *
 * For reminders sent about an invoice that isn't due yet, see
 * InvoicePaymentReminderNotification instead - both notifications listen on
...
final class InvoiceOverdueReminderNotification extends AbstractNotification implements ResendableNotificationInterface
{
     ...
     */
    private const SENT_TABLE = 'mod_ishost_invoice_overdue_reminder_sent';
     ...
     * clicks "Send Email" > an invoice overdue/reminder template on the
     * invoice's Summary tab - WHMCS's native InvoicePaymentReminder hook
     * only fires for its own automated cron, not for that manual action.
     *
     * @return Hooks[]
     */
     ...
     *                            concern - e.g. the invoice isn't overdue yet).
     */
    public function transformHookParams(array $whmcsHookParams): ?array
    {
        ...
        // This notification is only for reminders about invoices that ARE
        // overdue - InvoicePaymentReminderNotification handles the other case.
