...
final class MetaWebhookController
{
    ...
    private function processStatus(array $status): void
    {
        ...
        // Meta doesn't always include a `conversation` object on every status
        // event - confirmed on this account: neither "sent" nor "delivered"
        // events carry one, only `pricing` does. Without this fallback, the
        // conversations table (which the Approximate Total Charges estimate
        // is built from) never gets updated at all for accounts like this
        // one, even though billable/category data is available every time.
        //
        // Derived key: recipient phone number + calendar day. Not a perfect
        // match for Meta's real rolling 24h conversation window, but far
        // closer to actual per-conversation billing than counting every
        // message as its own conversation - multiple billable messages to
        // the same person on the same day are grouped together, same as
        // they would be in a single real Meta conversation in the common case.
        if (!$conversationId && $billable !== null) {
            $recipientKey = $status['recipient_id'] ?? $waMessageId;
            $conversationId = 'derived:' . $recipientKey . ':' . $eventAt->format('Y-m-d');
        }
