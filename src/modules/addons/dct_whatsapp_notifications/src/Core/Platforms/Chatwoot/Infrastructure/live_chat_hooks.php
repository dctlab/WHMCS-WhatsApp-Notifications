...
use Dct\HookNotification\Core\Platforms\Chatwoot\Application\LiveChatService;
    ...
    function (array $whmcsHookParams) {
        try {
            if (empty($whmcsHookParams['client'])) {
                if (
                    isset($_SESSION['dct_whatsapp_notifications']['cw_live_chat_logout']) &&
                    $_SESSION['dct_whatsapp_notifications']['cw_live_chat_logout'] === true
                ) {
                    unset($_SESSION['dct_whatsapp_notifications']['cw_live_chat_logout']);
