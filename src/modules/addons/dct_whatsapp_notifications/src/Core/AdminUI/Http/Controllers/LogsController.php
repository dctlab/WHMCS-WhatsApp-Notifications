...
use Dct\HookNotification\Core\AdminUI\Infrastructure\LogsRepository;
...
final class LogsController extends BaseController
{
    ...
    public function viewLogs(array $request): void
    {
        ...
        if ($currentPage > $totalPages && $totalPages > 0) {
            header('Location: ?module=dct_whatsapp_notifications&page=logs&pageN=1' . ($filter ? '&filter=' . urlencode($filter) : ''));
            exit;
        }
