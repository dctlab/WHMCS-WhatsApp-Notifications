...
namespace Dct\HookNotification\Core\NotificationReport\Infrastructure;
...
use Dct\HookNotification\Core\Shared\Infrastructure\Repository\BaseRepository;
use WHMCS\Database\Capsule;

final class NotificationReportRepository extends BaseRepository
{
    private const TABLE = 'mod_dct_hook_notification_reports';
    ...
    /**
     * Per-notification-type delivery breakdown (e.g. "InvoiceCreated: sent
     * 40, delivered 38, read 30, failed 1") for the Dashboard's Notification
     * Performance card. Same grouping pattern as getDeliveryStatusCounts()
     * above, just additionally grouped by notification code - a new,
     * read-only presentation query, not a change to any sending/processing
     * logic.
     *
     * @param DateTime|null $from
     * @param DateTime|null $to
     * @param int            $limit Caps how many distinct notification types are returned, ordered by total sent.
     *
     * @return array<int, array{notification: string, sent: int, delivered: int, read: int, failed: int, delivery_rate: float}>
     */
    public function getNotificationPerformance(?DateTime $from = null, ?DateTime $to = null, int $limit = 10): array
    {
        $query = $this->query->table(self::TABLE)
            ->where('status', NotificationReportStatus::SENT->value);
        ...
        foreach ($rows as $row) {
            ...
            if (!isset($byNotification[$notification])) {
            ...
            // Every SENT report counts toward the "sent" total, regardless
            // of its current delivery_status - "sent" here means "we
            // successfully handed it to the platform", separate from
            // whether it went on to be delivered/read/failed.
            $byNotification[$notification]['sent'] += $total;

            if (isset($byNotification[$notification][$status])) {
                $byNotification[$notification][$status] += $total;
            }
        }

        $result = array_values($byNotification);

        foreach ($result as &$row) {
            $row['delivery_rate'] = $row['sent'] > 0
                ? round(($row['delivered'] + $row['read']) / $row['sent'] * 100, 1)
                : 0.0;
        }
        unset($row);
    ...
    /**
     * Most recent reports across all notification types, for the
     * Dashboard's Recent Activity feed - deliberately not time-window
     * restricted (unlike getReportsForLastHour()/getTopNotificationsForLastHour(),
     * which are specifically "last hour" for a different purpose), just the
     * latest N regardless of when they happened.
     *
    ...
    public function getRecentReports(int $limit = 10): array
    {
        return $this->query->table(self::TABLE)
            ->orderBy('created_at', 'desc')
            ->limit($limit)
            ->get(['id', 'client_id', 'notification', 'status', 'delivery_status', 'created_at'])
            ->toArray();
    }

    /**
     * Sent-message count per calendar day within the range, for the
     * Dashboard's lightweight Message Activity chart. Single series (sent
     * count) rather than sent/delivered/read/failed per day - a genuine
     * simplification (Section 8 explicitly allows this when multiple
     * series would be more confusing than useful for a no-library chart),
     * not a missing feature.
     *
     * @return array<string, int> counts keyed by "Y-m-d", every day in the
     *                             range present (zero-filled), so the chart
     *                             never has to guess at missing days.
     */
    public function getDailyMessageActivity(DateTime $from, DateTime $to): array
    {
        $rows = $this->query->table(self::TABLE)
            ->where('status', NotificationReportStatus::SENT->value)
            ->where('created_at', '>=', $from->format('Y-m-d 00:00:00'))
            ->where('created_at', '<=', $to->format('Y-m-d 23:59:59'))
            ->select(
                $this->query::table(self::TABLE)->raw('DATE(created_at) as day'),
                $this->query::table(self::TABLE)->raw('COUNT(*) as total')
            )
            ->groupBy('day')
            ->get()
    ...
    {
        $query = $this->query->table('mod_dct_hook_notification_conversations')
            ->where('billable', 1);
        ...
        $rows = $query
            ->select('category', $this->query::table('mod_dct_hook_notification_conversations')->raw('COUNT(*) as total'))
            ->groupBy('category')
            ->get();
    ...
    public function recordInboundMessage(
        ...
        $existing = $this->query->table('mod_dct_hook_notification_conversations')
        ...
        if ($existing) {
            $this->query->table('mod_dct_hook_notification_conversations')
    ...
    public function insertMessage(
        ...
        if ($waMessageId) {
            $exists = $this->query->table('mod_dct_hook_notification_messages')
                ->where('wa_message_id', $waMessageId)
                ->exists();
    ...
    {
        // Fetches more rows than $limit up front, since this table can have
        // multiple rows per phone number (one per billing-conversation
        // window - see upsertConversation()'s docblock), then dedupes down
        // to one entry per contact below. Already ordered by most recent
        // first, so unique() naturally keeps each contact's latest row.
        $conversations = $this->query->table('mod_dct_hook_notification_conversations')
    ...
    public function upsertConversation(
        ...
        if (!$existing && $phoneNumber) {
            ...
            $synthetic = $this->query->table('mod_dct_hook_notification_conversations')
    ...
    {
        $query      = $this->query->table('mod_dct_hook_notification_conversations');
        $countQuery = $this->query->table('mod_dct_hook_notification_conversations');
    ...
    public function getConversationAnalytics(?DateTime $from = null, ?DateTime $to = null): array
    {
        ...
        $byCategory = (clone $query)
            ->select(
                'category',
                $this->query::table('mod_dct_hook_notification_conversations')->raw('COUNT(*) as total')
            )
            ->groupBy('category')
            ->get()
            ->toArray();
