...
namespace Dct\HookNotification\Core\Shared\Infrastructure\Setup;

use Dct\HookNotification\Core\Notification\Infrastructure\Repositories\NotificationRepository;
...
final class DatabaseUpgrade
{
    ...
    {
        $query = Capsule::table('mod_dct_hook_notification_configs')
    ...
    public static function v200(): void
    {
        ...
        Capsule::schema()->rename(
            'mod_dct_hook_notification_platform_settings',
            'mod_dct_hook_notification_configs'
        );
        ...
        $updateConfig = function ($setting, $platform, $value): void {
            Capsule::table('mod_dct_hook_notification_configs')
                ->insert([
                    'platform' => $platform,
                    'setting' => $setting,
                    'value' => $value,
        ...
        $assocs = Capsule::table('mod_dct_hook_notification_configs')
    ...
    {
        Capsule::connection()->statement('ALTER TABLE mod_dct_hook_notification_reports MODIFY client_id INT(10) NULL');
        Capsule::connection()->statement('ALTER TABLE mod_dct_hook_notification_reports MODIFY category VARCHAR(20) NULL');
        Capsule::connection()->statement('ALTER TABLE mod_dct_hook_notification_reports MODIFY category_id BIGINT UNSIGNED NULL');
    ...
    {
        $activeChatwootNotifs = json_decode(Capsule::table('mod_dct_hook_notification_configs')
    ...
    public static function v380(): void
    {
        ...
        $statement = $pdo->prepare(
            'CREATE TABLE IF NOT EXISTS mod_dct_hook_notification_localized_tpls (
    ...
    public static function v390(): void
    {
        ...
        $statement = $pdo->prepare(
            'ALTER TABLE `mod_dct_hook_notification_localized_tpls` ADD `platform` VARCHAR(255) NOT NULL AFTER `notif_code`;'
        );
    ...
    public static function v400()
    {
        try {
            ...
            $statement = $pdo->prepare(
                'CREATE TABLE IF NOT EXISTS mod_dct_hook_notification_bulks (
            ...
            $statement = $pdo->prepare(
                'CREATE TABLE IF NOT EXISTS mod_dct_hook_notification_notif_queue (
                    id INT PRIMARY KEY AUTO_INCREMENT,
                    bulk_id INT NULL,
                    client_id INT NOT NULL,
                    `status` VARCHAR(255) NOT NULL,
                    notif_code VARCHAR(255) NULL,
                    FOREIGN KEY (bulk_id) REFERENCES mod_dct_hook_notification_bulks(id) ON DELETE CASCADE
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;'
            );
            ...
            Capsule::connection()->statement('
                ALTER TABLE mod_dct_hook_notification_reports
            ...
            Capsule::connection()->statement('
                ...
                FOREIGN KEY (queue_id) REFERENCES mod_dct_hook_notification_notif_queue(id)
                ON DELETE SET NULL
            ');

            Capsule::connection()->statement('
                ALTER TABLE mod_dct_hook_notification_localized_tpls
                MODIFY COLUMN platform VARCHAR(255) NULL
            ');
            ...
            Capsule::table('mod_dct_hook_notification_localized_tpls')
                ->where('lang', 'en_GB')
                ->update(['lang' => 'en_001']);
    ...
    public static function v412(): void
    {
        try {
            $result = Capsule::connection()->statement('
                ALTER TABLE `mod_dct_hook_notification_reports` CHANGE `category_id` `category_id` BIGINT UNSIGNED NULL DEFAULT NULL;
            ');

            $result2 = Capsule::connection()->statement('
                ALTER TABLE `mod_dct_hook_notification_reports` CHANGE `category` `category` VARCHAR(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL;
            ');
    ...
    public static function v430(): void
    {
        try {
            $result1 = Capsule::connection()->statement('
                ALTER TABLE mod_dct_hook_notification_bulks
                ADD COLUMN platform_payload TEXT NULL AFTER platform
            ');
    ...
    {
        try {
            $columns = Capsule::schema()->getColumnListing('mod_dct_hook_notification_reports');
            ...
            if (!Capsule::schema()->hasTable('mod_dct_hook_notification_reports_idx_wa')) {
                ...
                $hasIndex = Capsule::connection()->select("
                    ...
                    AND TABLE_NAME = 'mod_dct_hook_notification_reports'
            ...
            Capsule::connection()->statement('
                CREATE TABLE IF NOT EXISTS mod_dct_hook_notification_conversations (
    ...
    {
        try {
            $columns = Capsule::schema()->getColumnListing('mod_dct_hook_notification_conversations');

            if (!in_array('billable', $columns, true)) {
                Capsule::connection()->statement('
                    ALTER TABLE mod_dct_hook_notification_conversations
            ...
            $hasIndex = Capsule::connection()->select("
                SELECT COUNT(1) as total FROM information_schema.STATISTICS
                WHERE TABLE_SCHEMA = DATABASE()
                AND TABLE_NAME = 'mod_dct_hook_notification_conversations'
    ...
    public static function v453(): void
    {
        try {
            Capsule::connection()->statement('
                CREATE TABLE IF NOT EXISTS mod_dct_hook_notification_messages (
                    id INT NOT NULL AUTO_INCREMENT,
     ...
     * mod_dct_hook_notification_conversations), so the Reports page and
    ...
    {
        try {
            // Must run first, before the hasTable() check below - see the
            // detailed reasoning on v500RenameTablesForModuleRename() itself.
            // If this weren't first, a webhook event arriving before the
            // admin reactivates the module post-rename would hit the
            // hasTable() check below, find no table under the new name yet,
            // and return early without ever getting a chance to rename the
            // real data sitting under the old name.
            self::v500RenameTablesForModuleRename();
            self::v512EnsureUtf8mb4();

            if (!Capsule::schema()->hasTable('mod_dct_hook_notification_reports')) {
                // Module was never activated; nothing to heal here.
                return;
            }

            $reportColumns = Capsule::schema()->getColumnListing('mod_dct_hook_notification_reports');
            ...
            $missingV450 = !in_array('delivery_status', $reportColumns, true)
                ...
                || !Capsule::schema()->hasTable('mod_dct_hook_notification_conversations');
            ...
            if (Capsule::schema()->hasTable('mod_dct_hook_notification_conversations')) {
                $conversationColumns = Capsule::schema()->getColumnListing('mod_dct_hook_notification_conversations');
            ...
            $reportColumns = $reportColumns ?? Capsule::schema()->getColumnListing('mod_dct_hook_notification_reports');
    ...
    /**
     * Renames every mod_lkn_hook_notification_* table to
     * mod_dct_hook_notification_* (module rename), preserving all existing
     * data via RENAME TABLE (atomic, no data copying, keeps auto-increment
     * values and indexes intact).
     *
     * Deliberately idempotent and safe to call from multiple places: only
     * renames a table if the old name exists AND the new name doesn't
     * already exist, so calling this more than once (or concurrently) is a
     * harmless no-op after the first successful run.
     *
     * IMPORTANT: this must run before ensureDeliveryTrackingSchema()'s own
     * self-heal checks - those check hasTable('mod_dct_hook_notification_*')
     * to decide whether a table is "missing" and needs creating fresh. If
     * this rename hasn't happened yet (e.g. a webhook event arrives before
     * the admin reactivates the module), that self-heal would see the new
     * name as missing and CREATE AN EMPTY table under it - and once that
     * empty table exists, this rename method's own guard (skip if the new
     * name already exists) would then permanently skip migrating the real
     * data sitting under the old name. Calling this first, from both
     * _upgrade() and the very start of ensureDeliveryTrackingSchema(),
     * closes that race regardless of which code path runs first.
     */
    public static function v500RenameTablesForModuleRename(): void
    {
        $renames = [
            'mod_lkn_hook_notification_bulks' => 'mod_dct_hook_notification_bulks',
            'mod_lkn_hook_notification_client_prefs' => 'mod_dct_hook_notification_client_prefs',
            'mod_lkn_hook_notification_configs' => 'mod_dct_hook_notification_configs',
            'mod_lkn_hook_notification_conversations' => 'mod_dct_hook_notification_conversations',
            'mod_lkn_hook_notification_localized_tpls' => 'mod_dct_hook_notification_localized_tpls',
            'mod_lkn_hook_notification_messages' => 'mod_dct_hook_notification_messages',
            'mod_lkn_hook_notification_notif_queue' => 'mod_dct_hook_notification_notif_queue',
            'mod_lkn_hook_notification_platform_settings' => 'mod_dct_hook_notification_platform_settings',
            'mod_lkn_hook_notification_reports' => 'mod_dct_hook_notification_reports',
        ];

        foreach ($renames as $oldName => $newName) {
            try {
                if (Capsule::schema()->hasTable($oldName) && !Capsule::schema()->hasTable($newName)) {
                    Capsule::connection()->statement("RENAME TABLE `{$oldName}` TO `{$newName}`");

                    lkn_hn_log(
                        'Module rename: table renamed',
                ...
                lkn_hn_log(
                    'Module rename: table rename failed',
                    ['old' => $oldName, 'new' => $newName],
    ...
    /**
     * Converts any of this module's tables that are still on an older,
     * emoji-incompatible charset (plain "utf8", which only supports 3-byte
     * UTF-8 characters - most emoji are 4-byte and get silently replaced
     * with "?" on insert) to utf8mb4.
     *
     * Every CREATE TABLE statement in this module has specified utf8mb4 for
     * a long time, but CREATE TABLE IF NOT EXISTS only runs once - a table
     * created a long time ago (even predating this specific migration
     * history) under an older charset would never get upgraded just because
     * the code creating NEW installs now says utf8mb4. This actively
     * inspects and converts existing tables instead of assuming the code's
     * current CREATE TABLE definition reflects live reality.
     *
     * Safe to run repeatedly: does nothing to a table that's already
     * utf8mb4.
     */
    public static function v512EnsureUtf8mb4(): void
    {
        $tables = [
            'mod_dct_hook_notification_reports',
            'mod_dct_hook_notification_conversations',
            'mod_dct_hook_notification_messages',
        ];
        ...
        foreach ($tables as $table) {
            try {
                ...
                $currentCollation = Capsule::table('information_schema.TABLES')
                    ->where('TABLE_SCHEMA', Capsule::connection()->getDatabaseName())
                    ->where('TABLE_NAME', $table)
                    ->value('TABLE_COLLATION');

                if ($currentCollation && !str_starts_with((string) $currentCollation, 'utf8mb4')) {
                    Capsule::connection()->statement(
                        "ALTER TABLE `{$table}` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"
                    );

                    lkn_hn_log(
                        'Charset fix: table converted to utf8mb4',
                        ['table' => $table, 'previous_collation' => $currentCollation],
                        []
                    );
                }
            } catch (Throwable $th) {
                lkn_hn_log(
                    'Charset fix: conversion failed',
                    ['table' => $table],
                    ['exception' => $th->__toString()]
                );
            }
    ...
    /**
     * Adds the (created_at, status) composite index approved after the
     * Phase 2 dashboard performance review, on the reports table -
     * supports the Dashboard's Notification Performance, Recent Activity,
     * and Daily Message Activity queries added in that phase, none of
     * which had a supporting index before this.
     *
     * Follows the exact same idempotency pattern already established in
     * v450() for this table's other indexes (idx_wa_message_id etc.):
     * MySQL has no reliable "ADD INDEX IF NOT EXISTS" across the versions
     * this module supports, so existence is checked directly against
     * information_schema.STATISTICS before adding it. Deliberately does
     * NOT reuse v450()'s outer hasTable('..._idx_wa') guard - that guard
     * checks a placeholder table name that never actually exists, so it
     * was already a no-op there; the inner information_schema check alone
     * is what actually provides idempotency, so that is all this uses.
     *
     * Column order is exactly (created_at, status), matching the approved
     * design - not (status, created_at) - since created_at leading is what
     * lets a plain "ORDER BY created_at DESC LIMIT 10" (Recent Activity)
     * use this index at all, which a status-first index could not do.
     *
     * Adds only this one index. Does not touch, rename, or remove any
     * existing index, column, primary key, or data.
     */
    public static function v531AddReportsCreatedAtStatusIndex(): void
    {
        try {
            if (!Capsule::schema()->hasTable('mod_dct_hook_notification_reports')) {
                return;
            ...
            $hasIndex = Capsule::connection()->select("
                ...
                WHERE TABLE_SCHEMA = DATABASE()
                AND TABLE_NAME = 'mod_dct_hook_notification_reports'
                AND INDEX_NAME = 'idx_created_at_status'
            ");
            ...
            if (($hasIndex[0]->total ?? 0) == 0) {
                ...
                lkn_hn_log(
                    'Phase 2 performance review: idx_created_at_status added',
                    [],
                    []
                );
            }
            ...
            lkn_hn_log(
                'Phase 2 performance review: idx_created_at_status failed',
                [],
                ['exception' => $th->__toString()]
            );
        }
